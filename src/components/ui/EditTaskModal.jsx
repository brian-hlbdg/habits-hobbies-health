...
export default function EditTaskModal({ item, onSave, onClose }) {
  const [title, setTitle]         = useState(item.title)
  const [category, setCategory]   = useState(item.category)
  const [context, setContext]     = useState(item.context || 'home')
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-zinc-900 rounded-t-2xl md:rounded-2xl p-5 slide-up shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800 dark:text-zinc-100">Edit task</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          >
            <span className="font-medium">{recurring ? 'Recurring task' : 'One-time task'}</span>
