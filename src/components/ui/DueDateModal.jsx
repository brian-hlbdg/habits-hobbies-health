...
export default function DueDateModal({ item, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-zinc-900 rounded-t-2xl md:rounded-2xl p-5 slide-up shadow-xl dark:shadow-zinc-950">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800 dark:text-zinc-100 text-sm truncate pr-4">Due date</h3>
        ...
        <p className="text-xs text-gray-500 dark:text-zinc-400 mb-2 truncate">{item.title}</p>
        <input
          type="date"
          value={date}
        ...
        <div className="flex gap-2 mt-4">
          {item.due_date && (
            <button
              onClick={handleClear}
              className="flex-1 rounded-lg border border-gray-200 dark:border-zinc-700 py-2.5 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950 transition"
            >
              Clear
            </button>
          )}
